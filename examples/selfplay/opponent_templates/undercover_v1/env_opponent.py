...
from base_llm_opponent import BaseLLMOpponent
...
class env_Opponent(BaseLLMOpponent):
    ...
    def _build_message(self, observation, phase: str) -> str:
        word = observation["word"]
        messages_dict = observation['messages']
        messages = "".join(f"{p}: {c}\n" for p, c in messages_dict.items())

        if phase == 'talk':
            message = (
                f"Now is the giving clues stage:\n"
                f"You are {self.player_name}. Your secret word is \"{word}\".\n"
                f"This round the other players say:\n{messages}"
                f"Now you give one clue (but never include the secret word). "
                f"You cannot repeat what other players have said.\n"
                f"You should finally output {self.player_name} and the clue in json, e.g.: "
                f"{{\"answer\": \"{self.player_name}: It's a fruit.\"}}\n"
            )
        else:  # vote
            message = (
                f"Now is the accusation stage:\n"
                f"You are {self.player_name}. Your secret word is \"{word}\".\n"
                f"This round the other players say:\n{messages}"
                f"Now guess what other players' secret words are and "
                f"vote which of the other players is the undercover.\n"
                f"You should finally output \"vote\" and the undercover name in json, e.g.: "
                f"{{\"answer\": \"vote: player_1.\"}}\n"
            )
        return message

    # ========== 固定部分：解析答案 ==========
    def _parse_answer(self, answer: str, phase: str, action_mask, info=None) -> int:
        if phase == "talk":
            ...
            try:
                action_str = re.findall(r'vote:\s+(.*)', answer)[0]
                action = int(re.findall(r'\d', action_str)[0])
            except Exception:
                # fallback 随机选
                print(f" {self.player_name} Vote error,  随机选")
                indices_of_ones = [i for i, v in enumerate(action_mask) if v == 1]
                return random.choice(indices_of_ones[:-1])
            # 合法性检查
    ...
    # ========== 开放部分：请求动作（可重载） ==========
    def _request_action(self, message: str, phase: str) -> str:
        """默认使用 LLM，可以被子类重写"""
    ...
    def _get_chat_action(self, action_mask, observation, info=None):
        phase = observation["phrase"]
        message = self._build_message(observation, phase)

        try:
            answer = self._request_action(message, phase)
            action = self._parse_answer(answer, phase, action_mask, info)
