...
def env(render_mode=None, undercover_mode='random'):
    internal_render_mode = render_mode if render_mode != "ansi" else "human"
    env = raw_env(render_mode=internal_render_mode,undercover_mode=undercover_mode)
    if render_mode == "ansi":
...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(
        self, render_mode: str | None = None, num_players: int = 5, num_undercovers: int = 1,undercover_mode: str = 'random'
    ):
        super().__init__()
        ...
        self.word_list = open(f"{file_dir}/words.txt","r").readlines()
    ...
    def set_undercovers(self):
        """
        根据 undercover_mode 参数设置卧底玩家。
        """
        all_players = list(range(self.num_players))

        if self.undercover_mode == 'fixed':
            # 模式2：指定卧底为玩家0 (player_0)
            # 确保玩家数量足够，否则会抛出错误
            if self.num_players < 2:
                raise ValueError("固定模式需要至少2名玩家。")
            self.undercover_set = {0}
            
        elif self.undercover_mode == 'semi':
            # 模式3：半随机，卧底是除了玩家0之外的随机一位
            # 从所有玩家中移除玩家0
            other_players = all_players.copy()
            if 0 in other_players:
                other_players.remove(0)
            # 从剩余玩家中随机选择
            self.undercover_set = set(random.sample(other_players, self.num_undercovers))

        else: # 'random' 或其他任何未指定的值
            # 模式1：随机，卧底玩家在随机位置
            self.undercover_set = set(random.sample(all_players, self.num_undercovers))

        self.non_undercover_set = set(list(range(self.num_players))) - self.undercover_set

        # 打印分配结果，方便调试
        # print(f"卧底玩家: {self.undercover_set}")
        # print(f"非卧底玩家: {self.non_undercover_set}")
