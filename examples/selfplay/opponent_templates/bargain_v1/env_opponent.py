...
class env_Opponent(BaseLLMOpponent):
    ...
    # ========== 文本构建 ==========
    def _build_message(self, observation) -> str:
        ...
        message += (
            f"There are {item_nums[0]} hats, {item_nums[1]} balls and {item_nums[2]} apples.\n"
            f"The value for each hat is {value[0]}, for each ball is {value[1]} and for each apple is {value[2]}.\n"
        )
        ...
        if oppo_plan is not None:
            ...
            message += (
                f"He will get {oppo_plan[0]} hats, {oppo_plan[1]} balls and {oppo_plan[2]} apples.\n"
                f"And you will get {item_nums[0] - oppo_plan[0]} hats, "
                f"{item_nums[1] - oppo_plan[1]} balls and "
                f"{item_nums[2] - oppo_plan[2]} apples, the total value is "
                f"{(item_nums[0] - oppo_plan[0]) * value[0] + (item_nums[1] - oppo_plan[1]) * value[1] + (item_nums[2] - oppo_plan[2]) * value[2]}.\n"
                f"Do you agree with his plan? If you agree, You should only output {self.player_name} and \"Deal\", "
                f"for example: \"{self.player_name}: Deal.\".\n\n"
                f"If you do not agree, you should say something to bargain with your opponent first and then output "
                f"{self.player_name} and your plan as the following format:\n"
            )
        else:
            message += (
                f"You should say something to bargain with your opponent first and then output "
                f"{self.player_name} and your plan as the following format:\n"
            )

        message += (
            f"\"{self.player_name}: x hats y balls z apples\". x, y, z are Arabic number.\n"
            f"For example:\"I'd like 1 hats, 2 balls and 0 apples. {self.player_name}: 1 hats 2 balls 0 apples\".\n"
            f"Do not tell you opponent your value of each item, try to maximum the value you get."
        )
        return message
    

    def _parse_answer(self, answer: str, action_mask, info=None) -> int:
        answer = answer.replace("Player", "player").replace("\\", "")
        match = re.findall(rf'(.*){self.player_name}:\s+(.*)', answer)

        if len(match) == 0:  # 没有匹配上明确的格式
            try:
    ...
    def _get_chat_action(self, action_mask, observation, info=None):
        message = self._build_message(observation)
        try:
            answer = self._request_action(message)
            action = self._parse_answer(answer, action_mask, info)
