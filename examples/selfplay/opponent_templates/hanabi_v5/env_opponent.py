...
class env_Opponent(BaseLLMOpponent):
    ...
    def _obs_lastplay(self,observation):
        ...
        lastplayed = None
        if env_Opponent.opponent_action is not None:
            if int(self.pos2act[env_Opponent.opponent_action]) == 2 or int(self.pos2act[env_Opponent.opponent_action]) == 3:
                lastplayed = self._decode_cards("last played card", observation)
            if int(self.pos2act[env_Opponent.opponent_action]) == 0 or int(self.pos2act[env_Opponent.opponent_action]) == 1:
    ...
    def _build_message(self, observation: Dict, action_mask: List[int]) -> str:
        """
        根据观察值构建发送给 LLM 的消息。
        """
        ...
        if env_Opponent.opponent_action is not None:
            message += f"Your opponent's action is {env_Opponent.opponent_action}\n"
            
        message += f"The card that opponent last played or discarded is {self.lastplayed}\n"
        #message += "You should finally output your action. For example: 'Reveal Yellow Cards for another player'\n"
        ...
        message += f" You should finally output your action in a JSON format. For example:\n"
        message += f"{{\"Action\": \"Reveal Yellow Cards for another player\"}}\n"

        
        return message

    # ========== 新增部分：解析答案 ==========
    def _parse_answer(self, answer: str, action_mask: List[int]) -> int:
        """解析 LLM 的回答并返回一个合法的动作索引。
        answer: "Action: Reveal Yellow Cards for another player"
        """
        # 构建一个正则表达式，用于匹配所有合法的动作描述
        # 假设 self.act2des 是 {0: "desc1", 1: "desc2", ...}
        # 假设 self.pos2act 是 {"desc1": 0, "desc2": 1, ...}
        valid_actions_str = "|".join(re.escape(desc) for desc in self.pos2act.keys())
        pattern = rf"Action:\s*({valid_actions_str})"
        
        match = re.search(pattern, answer, re.IGNORECASE)
        
        if match:
            action_desc = match.group(1).strip().rstrip('.')
            action = int(self.pos2act[action_desc])
            
            # 合法性检查：确保提取的动作是当前回合允许的
            if action_mask[action] == 1:
                #print(f"Extracted valid action: {action_desc} (ID: {action})")
                env_Opponent.opponent_action = action_desc  # 更新上一轮动作记录
                return action
            else:
                raise ValueError(f"Extracted action '{action_desc}' is illegal in the current state.")
        else:
            raise ValueError("Could not find a valid action pattern in the LLM's response.")


    # ========== 开放部分：请求动作（可重载） ==========
    def _request_action(self, message: str) -> str:
        """默认使用 LLM，可以被子类重写。"""
        # phase 参数是为了与 undercover 接口保持一致，此处未使用
        return self._get_chat_answer(message=message)
    ...
    #                 env_Opponent.opponent_action = pos
    #                 break
    #             else:
    #                 print("未找到匹配。")
    #                 current_retry += 1 
    ...
    #         illegal_count += 1
    #     env_Opponent.round_retry += current_retry
    #     env_Opponent.round_illegal += illegal_count
    #     return action
...
if __name__ == "__main__":
    ...
    opponent1 = env_Opponent(
        opponent_info = info
    )
    opponent2 = env_Opponent(        
        opponent_info = info
    )
